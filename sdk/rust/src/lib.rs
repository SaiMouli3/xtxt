...
// An unrecognised language is escaped and returned unchanged.
...
fn lang_spec(name: &str) -> Option<LangSpec> {
    let name = match name {
        "js" | "jsx" | "ts" | "typescript" | "tsx" | "mjs" => "javascript",
        "py" => "python",
        "rs" => "rust",
        "sh" | "bash" | "zsh" => "shell",
        "cpp" | "c++" | "h" | "hpp" | "cc" => "c",
...
fn hl_word(c: u8) -> bool {
    c == b'_' || c.is_ascii_alphanumeric()
}

fn string_end(s: &[u8], start: usize) -> usize {
    let quote = s[start];
    let mut i = start + 1;
    while i < s.len() {
        match s[i] {
...
/// Escaped HTML with token spans around comments, strings, numbers and keywords.
pub fn highlight_html(source: &str, language: &str) -> String {
    let spec = match lang_spec(language.trim().to_lowercase().as_str()) {
        Some(s) => s,
        None => return escape_str(source),
    };
    ...
    macro_rules! flush {
        ($upto:expr) => {
            if $upto > plain {
                out.push_str(&escape_str(&source[plain..$upto]));
            }
        };
    }
    macro_rules! span {
        ($cls:expr, $from:expr, $to:expr) => {{
            out.push_str(concat!("<span class=\"tok-", $cls, "\">"));
            out.push_str(&escape_str(&source[$from..$to]));
            out.push_str("</span>");
        }};
    }
    ...
    while i < b.len() {
        ...
        if !spec.block.0.is_empty() && source[i..].starts_with(spec.block.0) {
            let from = i + spec.block.0.len();
            let end = source[from..]
                .find(spec.block.1)
                .map(|k| from + k + spec.block.1.len())
                .unwrap_or(b.len());
            flush!(i);
            span!("com", i, end);
            i = end;
        ...
        if spec.quotes.as_bytes().contains(&b[i]) {
        ...
        if hl_word(b[i]) && (i == 0 || !hl_word(b[i - 1])) {
            let mut end = i;
            while end < b.len() && hl_word(b[end]) {
                end += 1;
            }
            if spec.keywords.contains(&&source[i..end]) {
                flush!(i);
                span!("kw", i, end);
                plain = end;
            }
